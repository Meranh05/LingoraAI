...
function CreatePathForm({ saving, onCreate }: { saving: boolean; onCreate: (value: Record<string, unknown>) => void }) {
  ...
  return <FormCard title="Tạo lộ trình mới" description="Khung lớn chứa nhiều checkpoint theo level."><form onSubmit={submit} className="grid gap-4"><Field label="Slug"><Input name="slug" placeholder="english-adventure-b1" required /></Field><LocalizedFields prefix="title" label="Tên lộ trình" required /><LocalizedFields prefix="description" label="Mô tả" multiline /><div className="grid grid-cols-2 gap-3"><Field label="Level đích"><NativeSelect name="targetLevel" values={levels} /></Field><Field label="Số giờ"><Input name="estimatedHours" type="number" defaultValue="20" min="1" /></Field></div><PublishSwitch /><Submit saving={saving} label="Tạo lộ trình" /></form></FormCard>;
}
...
function CreateQuestionForm({ units, paths, saving, onCreate }: { units: StudioData["units"]; paths: StudioData["paths"]; saving: boolean; onCreate: (value: Record<string, unknown>) => void }) {
  ...
  return <FormCard title="Custom câu hỏi theo level" description="Gắn trực tiếp câu hỏi vào checkpoint và tự thêm vào pool thử thách phù hợp."><form onSubmit={submit} className="grid gap-4"><Field label="Checkpoint"><select name="unitId" required className="h-9 rounded-md border bg-transparent px-3 text-sm">{units.map((unit) => <option key={unit.id} value={unit.id}>{text(paths.find((path) => path.id === unit.path_id)?.title ?? {})} / {unit.position}. {text(unit.title)}</option>)}</select></Field><div className="grid grid-cols-3 gap-3"><Field label="Level"><NativeSelect name="difficulty" values={levels} /></Field><Field label="Kỹ năng"><NativeSelect name="skill" values={skills} /></Field><Field label="Loại"><NativeSelect name="questionType" values={["multiple_choice","true_false","fill_blank","match_meaning","sentence_order","short_answer","essay","dictation","speaking"]} /></Field></div><LocalizedFields prefix="prompt" label="Câu hỏi" required multiline /><Field label="Đoạn đọc / ngữ cảnh"><Textarea name="passage" /></Field><Field label="Audio URL"><Input name="audioUrl" type="url" placeholder="https://..." /></Field><Field label="Lựa chọn, mỗi dòng một đáp án"><Textarea name="options" placeholder={"Option A\nOption B\nOption C"} /></Field><Field label="Đáp án đúng / câu hoàn chỉnh"><Input name="answer" required /></Field><LocalizedFields prefix="explanation" label="Giải thích" multiline /><PublishSwitch /><Submit saving={saving} label="Thêm câu hỏi" /></form></FormCard>;
}
...
function LocalizedFields({ prefix, label, required = false, multiline = false }: { prefix: string; label: string; required?: boolean; multiline?: boolean }) {
  ...
  return <div className="grid gap-3 md:grid-cols-2"><Field label={`${label} · VI`}><Control name={`${prefix}Vi`} required={required} /></Field><Field label={`${label} · EN`}><Control name={`${prefix}En`} /></Field><Field label={`${label} · JA`}><Control name={`${prefix}Ja`} /></Field><Field label={`${label} · TH`}><Control name={`${prefix}Th`} /></Field></div>;
}
